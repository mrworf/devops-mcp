---
name: devops-mcp-next-milestone
description: Use for this devops-mcp project when the user asks to start, implement, continue, complete, or inspect the next MVP milestone, including phrases like "start next milestone", "implement next MVP milestone", or "continue devops-mcp milestone"; reads docs/milestones/status.yaml and the selected milestone plan, implements only that milestone, and updates milestone status.
---

# DevOps MCP Next Milestone

Use this skill only inside the `devops-mcp` project.

## Workflow
1. Read `docs/AGENTS.md`.
2. Run `node .codex/skills/devops-mcp-next-milestone/scripts/next_milestone.mjs` to identify the first pending milestone, unless the user names a specific milestone.
...
8. Mark the milestone complete with `node .codex/skills/devops-mcp-next-milestone/scripts/mark_complete.mjs <id> --notes "<short validation summary>"`.
