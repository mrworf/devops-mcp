...
- Authenticated downstream work is limited by `limits.max_service_requests_inflight` globally (default `32`) and `limits.max_service_requests_inflight_per_subject` per authenticated subject (default `4`). Excess work returns a structured `capacity_exceeded` tool error before reference redemption, credential substitution, or downstream I/O. Slots are released after every success or failure.
