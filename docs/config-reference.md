...
- MCP transports are per-configuration routing state, capped globally by `limits.max_mcp_transports` (default `1000`) and per authenticated subject by `limits.max_mcp_transports_per_subject` (default `10`). A subject may initialize `limits.max_mcp_initializations_per_subject` transports (default `20`) per `limits.mcp_initialization_window` (default `1m`); rate state is bounded by `limits.max_mcp_initialization_records` (default `10000`). Idle transports are reaped after `limits.mcp_transport_idle_ttl` (default `30m`). Capacity and rate denials return HTTP `429`. Every request is re-authenticated and must match the transport's subject, but transport IDs remain routing state rather than a standalone authorization boundary.
