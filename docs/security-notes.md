...
- Secretlint scans response header values and UTF-8 body source text. A separate case-insensitive regex catalog protects sensitive response header names and complete string values in tolerant JSON/environment shapes even when endpoint Secretlint is disabled. JSON is never deserialized or reserialized, so all bytes outside replacement ranges remain unchanged.
